const ValidatorRegister = (options) => {
        ...
        options.rules.forEach(function (rule) {
            ...
            Array.from(inputElement).forEach(function (inputElement) {
                ...
                inputElement.oninput = function () {
                    let errorElement = getParent(inputElement, options.formGroupSelector).querySelector('.register-form-message');
                    getParent(inputElement, options.formGroupSelector).classList.remove('invalid');
                    errorElement.textContent = '';
                }
            });
...
ValidatorRegister.isRequired = function (selector, message) {
    return {
        selector: selector,
        test: function (value) {
...
ValidatorRegister.isEmail = function (selector, message) {
    return {
        selector: selector,
        test: function (value) {
...
ValidatorRegister.minLength = function (selector, min, message) {
    return {
        selector: selector,
        test: function (value) {
...
ValidatorRegister.isConfirmed = function (selector, getConfirmValue, message) {
    return {
        selector: selector,
        test: function (value) {
